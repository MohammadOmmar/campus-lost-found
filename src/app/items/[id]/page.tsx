...
export default async function ItemDetailPage({ params, searchParams }: ItemDetailPageProps) {
  ...
  return (
    <div className="container-tight pt-24 pb-12">
      <Link href="/items" className="inline-flex items-center gap-2 text-[13px] text-muted-foreground hover:text-foreground transition-colors mb-8">
        <ArrowLeft className="w-3.5 h-3.5" /> Back to Browse
      </Link>

      <div className="grid lg:grid-cols-2 gap-8 lg:gap-12">
        <div className="aspect-square bg-secondary/30 rounded-2xl flex items-center justify-center overflow-hidden border border-border/30">
          {item.image_url ? (
        ...
        <div className="flex flex-col">
          ...
          <div className="space-y-3 mb-8">
            <div className="flex items-center gap-2 text-[14px] text-muted-foreground"><MapPin className="w-4 h-4" /><span>{item.location}</span></div>
            <div className="flex items-center gap-2 text-[14px] text-muted-foreground"><Calendar className="w-4 h-4" /><span>{formatDate(item.date_lost_found)}</span></div>
          </div>

          <div className="mb-8">
            <h3 className="text-[12px] text-muted-foreground uppercase tracking-wide mb-3">Description</h3>
            <p className="text-[14px] text-foreground leading-relaxed whitespace-pre-wrap">{item.description}</p>
          </div>

          <div className="mb-8">
            <h3 className="text-[12px] text-muted-foreground uppercase tracking-wide mb-3">Category</h3>
            <p className="text-[14px] text-foreground">{item.category}</p>
          </div>
          {isOwner && item.type === 'FOUND' && privateNote && (
            <div className="mb-8 rounded-2xl border border-border/30 bg-secondary/20 p-5">
              <p className="text-[12px] text-muted-foreground mb-2 inline-flex items-center gap-2"><Lock className="h-3 w-3" /> Private verification · only you can see this</p>
              <p className="text-[14px] whitespace-pre-wrap">{privateNote}</p>
            </div>
          )}

          {myClaim && (
            <div className="mb-8 rounded-2xl border border-border/30 bg-card p-5">
              <p className="text-[13px] text-muted-foreground">Your claim status: <span className="font-medium text-foreground capitalize">{myClaim.status}</span></p>
            </div>
          )}
          ...
          {isOwner && item.type === 'FOUND' && (
            <div className="mb-8">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-[12px] text-muted-foreground uppercase tracking-wide">Claims{pendingCount > 0 ? ` · ${pendingCount} awaiting review` : ''}</h3>
              </div>
          ...
          <div className="mt-auto pt-6 border-t border-border/30">
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              {!isOwner && !canClaim && item.status !== 'OPEN' && (<p className="text-[13px] text-muted-foreground">This item is no longer accepting claims.</p>)}
            </div>
          </div>
        </div>
      </div>

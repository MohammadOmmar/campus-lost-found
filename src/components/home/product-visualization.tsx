...
import { MapPin, Calendar, ArrowRight, Zap, Lock } from 'lucide-react'

const easeOut = [0.4, 0, 0.2, 1] as const

function Card({ badge, badgeColor, children, active = false }: {
  badge: string
  badgeColor: string
  ...
  return (
    <motion.div
      className={`bg-base-850 border rounded-lg p-5 transition-all duration-300 ${
        active ? 'border-heritage-border/60' : 'border-border-subtle hover:border-border-default'
      }`}
      whileHover={{ y: -2 }}
    >
  ...
  return (
    <div className="w-full max-w-5xl mx-auto">
      <div className="hidden lg:block">
        <div className="flex items-start justify-between gap-2">
          <motion.div className="flex-1" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2, ease: easeOut }}>
            <Card badge="LOST" badgeColor="!text-status-lost !border-status-lost/20 !bg-status-lost/10">
          ...
          <motion.div className="flex-1" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.5, ease: easeOut }}>
            <Card badge="POTENTIAL MATCH" badgeColor="!text-heritage !border-heritage-border !bg-heritage-muted" active>
          ...
          <motion.div className="flex-1" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.8, ease: easeOut }}>
            <Card badge="VERIFICATION" badgeColor="!text-status-verification !border-status-verification/20 !bg-status-verification/10">
          ...
          <motion.div className="flex-1" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 1.1, ease: easeOut }}>
            <Card badge="RETURNED" badgeColor="!text-status-returned !border-status-returned/20 !bg-status-returned/10">
      ...
      <div className="lg:hidden space-y-3">
        ...
        <motion.div className="bg-base-850 border border-heritage-border/60 rounded-lg p-4" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.2, ease: easeOut }}>
          <div className="flex items-start">
            <div className="min-w-0">

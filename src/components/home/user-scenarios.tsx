    ...
    <section className="section-spacing border-t border-border-subtle bg-base-900/50 relative">
      <div className="container-tight">
        <div className="text-center mb-16">
          ...
          <motion.h2 className="text-h1 mt-4 mb-4" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, ease: easeOutExpo }}>
          ...
          <div className="inline-flex items-center gap-1 p-1 bg-base-850 border border-border-subtle rounded-md">
            {scenarios.map((scenario) => (
              <button key={scenario.id} onClick={() => setActiveTab(scenario.id)} className={`relative px-5 py-2 rounded-sm text-[13px] font-medium transition-colors duration-200 ${activeTab === scenario.id ? 'text-text-primary' : 'text-text-muted hover:text-text-secondary'}`} aria-selected={activeTab === scenario.id} role="tab">
                {activeTab === scenario.id && (<motion.div className="absolute inset-0 bg-accent/10 rounded-sm border border-accent/20" layoutId="activeTab" transition={{ duration: 0.25, ease: easeOutExpo }} />)}
        ...
        <AnimatePresence mode="wait">
          <motion.div key={activeTab} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.3, ease: easeOutExpo }} className="bg-base-850/60 border border-border-subtle rounded-xl p-8 md:p-12">
            <div className="grid md:grid-cols-2 gap-8 md:gap-12 items-center">
              ...
              <div className="space-y-4">
                {activeScenario.steps.map((step, index) => (
                  <motion.div key={index} className="flex items-start gap-4" initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3, delay: index * 0.08, ease: easeOutExpo }}>
